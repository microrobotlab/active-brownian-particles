...
function stat_analysis1(a,b,R,pathf)
  f= pathf*".csv"
  f1= pathf*"_p.csv"              # file to store number of particles at equators and poles
  f2= pathf*"_pf.png"
...
dt= 1#1e-2
i= (time/dt)
eθ = atan(b/a)                 # Angle at which area at equator and poles will be same
pf_factor = R*R
...
for i=1:length(gdf)                                                                                                                                                                                                                                
   ...
   global  count2=0 
    ...
    #########################################################################################
    # PLOTING IN TERM OF NUMBER OF PARTICLES
     #=
  ...
  savefig("data_ellipse_N1.png")
    =# 
    #########################################################################################
    # PLOTING IN TERM OF PACKING FRACTION 

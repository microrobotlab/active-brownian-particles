...
function update_heun(abpe::ABPE, matrices::Tuple{Matrix{Float64}, BitMatrix, BitMatrix}, δt::Float64, forward::Bool, offcenter::Float64, range::Float64, int_func::Function, int_params...) where {ABPE <: ABPsEnsemble}
    ...
    #intermediate step
    if (!isapprox(offcenter,0.0))
        f_i, t_i = force_torque(pθ[1], pθ[2], abpe.R, abpe.L, forward, offcenter, range, int_func, int_params...)
    else
        f_i = interactions_range(pθ[1], abpe.R, abpe.L, range, abpe.Np, int_func, int_params...)
        t_i = zeros(abpe.Np)
    end 

    δp_i .= sqrt.(2*δt*abpe.DT)*randn(abpe.Np,2) .+ δt.*abpe.v.*[cos.(abpe.θ) sin.(abpe.θ)] .+ δt*1e-6f_i/γₜ 
    δθ_i .= sqrt(2*abpe.DR*δt)*randn(abpe.Np) .+ δt.*abpe.ω .+ δt*1e-18t_i/γᵣ
    ...
    if (!isapprox(offcenter,0.0))
        ...
        f_f = interactions_range(pθ[1], abpe.R, abpe.L, range, abpe.Np, int_func, int_params...)
        t_f = zeros(abpe.Np)
    end 

    δp .= sqrt.(2*δt*abpe.DT)*randn(abpe.Np,2) .+ δt.*abpe.v.*[cos.(abpe.θ) sin.(abpe.θ)] .+ δt*1e-6f_f/γₜ 
    δθ .= sqrt(2*abpe.DR*δt)*randn(abpe.Np) .+ δt.*abpe.ω .+ δt*1e-18t_f/γᵣ

    pθ = (position(abpe), orientation(abpe)) .+ (δp, δθ)

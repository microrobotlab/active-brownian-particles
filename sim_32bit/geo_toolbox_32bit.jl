...
"""
    pwdist(x::Vector{Float32}[, y::Vector{Float32}])
...
```
"""
pwdist(x::Vector{Float32}) =[b-a for a in x, b in x]

pwdist(x::Vector{Float32}, y::Vector{Float32}) = [b-a for a in x, b in y] 

"""
    radial_directions(xy::Array{Float32,2})
...
```
"""
function radial_directions(xy::Array{Float32,2})
...
"""
    angdiff(ϕ1::Float32, ϕ2::Float32)
    angdiff(ϕ1::Array{Float32,1}, ϕ2::Array{Float32,1})
...
```
"""
function angdiff(ϕ1::Float32, ϕ2::Float32)
	Δϕ = abs(ϕ1-ϕ2) % Float32(2π)
	(Δϕ < Float32(2π) - Δϕ) ? diff = Δϕ : diff = Float32(2π) - Δϕ
	return diff
end

function angdiff(ϕ1::Array{Float32,1}, ϕ2::Array{Float32,1})
	Δϕ = pwdist(ϕ1, ϕ2) .% Float32(2π)
	pirotation!(Δϕ)
	Δϕ[Δϕ .> Float32(2π) .- Δϕ] .= Float32(2π) .- Δϕ[Δϕ .> Float32(2π) .- Δϕ]
	return Δϕ
end

"""
    pirotation(θ::Array{Float32})

Shifts all the angles from any domain to [0, Float32(2π)]
...
function pirotation(θ::Real)
	θr = θ%Float32(2π)
	θr<0 ? θr+Float32(2π) : θr
end

function pirotation(θ::Array{Float32})
	θr = θ.%Float32(2π)
	θr[θr.<0].+=Float32(2π)
	return θr
end
...
Shifts all the angles from any domain to [0, Float32(2π)], reassigning.
```
"""
function pirotation!(θ::Array{Float32})
	θ.%=Float32(2π)
...
"""
d2(xy::Array{Float32,2}) = sqrt.(sum(abs2, xy, dims=2))
d2(xy) = sqrt.(sum(abs2, xy, dims=2))


"""
    ellipse_sectors(a::Float32,b::Float32,r::Float32,shell::Bool)
...
```
"""
function ellipse_sectors(a::Float32,b::Float32,r::Float32,shell::Bool) # area of intersection between ellipse and circle with r >b
...
"""
    intersection_area_el(a::Float32,b::Float32,r::Float32,shell::Bool)

Compute the area of the shell between R₁ and R₂ inside the ellipse
```
"""
function intersection_area_el(a::Float32,b::Float32,R₁::Float32,R₂::Float32) #a larger semiaxis, b smaller semiaxis, R₁ smaller circle radius
    if R₂ <= b
        area = pi*(R₂^2-R₁^2)
    end
...
"""
    intersection_area_sq(r::Float32,L::Float32)

Compute the area of the intersection between a circle of radius r and a square of side L
```
"""
function intersection_area_sq(r::Float32, L::Float32)
    l = L/2
...
"""
    xy_to_points(xy::Array{Float32,2})
...
```
"""
function xy_to_points(xy::Array{Float32,2})
    return [Point2(row) for row in eachrow(xy)]
end

"""
    points_to_xy(points::Vector{Point{2, Float32}})
...
```
"""
function points_to_xy(points::Vector{Point{2, Float32}})
    xy = zeros(Float32, length(points), 2)
...
"""
    round_point(p::Point{2,Float32}, tol=1e-8)
...
```
"""
function round_point(p::Point{2,Float32}, tol=1e-8)
    return round.(p, digits=ceil(Int, -log10(tol)))
end

"""
    get_adjacency_from_points(cells::Vector{Vector{Point{2,Float32}}}; tol=1e-8)

Returns a set of tuples representing the adjacency of Voronoi cells with particles at their centers.
"""
function get_adjacency_from_points(cells::Vector{Vector{Point{2,Float32}}}, Np::Int; tol=1e-8)
    # Step 1: Normalize and index all unique points
    point_to_index = Dict{Point{2,Float32}, Int}()
    index_counter = 1
...
"""
    find_boundary_points(cells::Vector{Vector{Point{2,Float32}}})

Finds the boundary points of Voronoi cells that are outside the simulation box and returns their periodic projections.
"""
function find_boundary_points(xy::Array{Float32, 2}, cells)
    xy_periodic_projection = Array{Float32, 2}(undef, 0, 2)

...
function initABPE(Np::Int64, L::Real, R::Real, T::Real, vd::Union{Real,Array{Float32,1},Distribution}, ωd::Union{Real,Array{Float32,1},Distribution}, int_func::Function, offcenter::Float32, int_params...; η::Float32=1f-3)
...
function update_heun(abpe::ABPE, matrices::Tuple{Matrix{Float32}, BitMatrix, BitMatrix}, δt::Float32, offcenter::Float32, int_func::Function, int_params...) where {ABPE <: ABPsEnsemble}
    ...
    if (!isapprox(offcenter,0.f0))
        ...
        f_f = interaction_voronoi(pθ_i..., oc_length, abpe.L, abpe.Np, int_func, int_params...)
        t_f = zeros(abpe.Np)
    end
...
function get_force_from_adjacency(adjacency::Set{Tuple{Int64, Int64}}, xy::Array{Float32, 2}, Np::Int, int_func::Function, int_params...)
    ΣFtot = zeros(Np, 2)
    for a in adjacency
        # Since tuples are ordered and the set is composed starting from points inside the simulation box, we are sure that a[1] <= Np
        dxy = xy[a[1],:] - xy[a[2],:]
...
function interaction_voronoi(xy::Array{Float32, 2}, θ::Array{Float32,1}, oc::Float32, L::Real, Np::Int, int_func::Function, int_params...)
    #Step 1: Get the tessellation
...
function force_torque_voronoi(xy::Array{Float32,2}, θ::Array{Float32,1}, L::Real, oc::Float32, int_func::Function, int_params...) #Forces are retuned in μN, torques in μN×μm

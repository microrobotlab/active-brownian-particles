...
mainname = @sprintf "R=%.1f v=%.1f a=%.1f b=%.1f pf=%.4f dt=%.0e Nt=%.0e" R v a b packing_fraction Delta_t Nt
...
for i=1:ICS
    ...
    start_sim = now()
    graph_wall = multiparticleE(Np,L,R,Uniform(0.,10.),Normal(0.,0.025),Nt,Delta_t) # has values of x and y position in each frame in graph_wall[1]

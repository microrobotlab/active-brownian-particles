using DataFrames, CSV, Statistics,  FFTW
...
f1= path*"running_avg_FFT.png"
f2=path*"running_avg_curve.png"
...
df_avg= running_avg(df,window_size)

start_frame= 1000
end_frame= 10000
fs=1.0
freq= fftshift(fft(df_avg[start_frame:end_frame,:running_avg]))
freqs = fftshift(fftfreq(length(df_avg[start_frame:end_frame,:running_avg]), fs))

k= plot(freqs,abs.(freq), xlimit=(-0.01,0.01), ylimit=(0.02,1000))#,seriestype=:scatter)
